 ...
 */
export async function applySoftspots({ svgRoot, svgViewBox, softspots, definition }) {
	if (!softspots?.definitions?.length) return { detectedSoftspots: [] };

	const shouldOutline = softspots.outline !== false;
	const shouldFill = softspots.fill === true;
	const smartsArray = Array.isArray(softspots.definitions) ? softspots.definitions : [];

	const { smartsMetadata, detectedSoftspots } = await runSmartsSearch(smartsArray, definition);
	...
	if (smartsMetadata.length > 0) {
		...
		for (const meta of smartsMetadata) {
			const { color, name: ssName, id: ssId, softspots: ssSpots } = meta;
