	...
	switch (desc.type) {
		case 'path': {
			...
			if (p.children && p.children.length === 1) {
				...
				if (desc.hasStroke) { child.strokeColor = 'black'; child.strokeWidth = desc.strokeWidth || 1; }
				child.closed = desc.closed;
				return child;
			}
			if (desc.hasFill) p.fillColor = 'black';
			if (desc.hasStroke) { p.strokeColor = 'black'; p.strokeWidth = desc.strokeWidth || 1; }
			return p;
		}
		case 'circle': {
			const c = new scope.Path.Circle(new scope.Point(desc.cx, desc.cy), desc.r);
			if (desc.hasFill) c.fillColor = 'black';
			if (desc.hasStroke) { c.strokeColor = 'black'; c.strokeWidth = desc.strokeWidth || 1; }
			return c;
		}
		case 'ellipse': {
			...
			if (desc.hasStroke) { e.strokeColor = 'black'; e.strokeWidth = desc.strokeWidth || 1; }
			return e;
		}
		case 'line': {
		...
		case 'rect': {
			...
			if (desc.hasStroke) { r.strokeColor = 'black'; r.strokeWidth = desc.strokeWidth || 1; }
			return r;
		}
		default:
			return null;
...
function computeOutline(shapeDescriptors, viewBox, options = {}) {
	...
	for (const openPath of openPaths) {
		try {
			...
			const strokeShape = PaperOffset.offsetStroke(openPath, Math.max(sw, strokeExpand) / 2, {
				join: 'round', cap: 'round', limit: 4, insert: false
			});
	...
	if (!items.length) { cleanup(); return []; }
	...
	function boundsIntersect(a, b) {
		const A = a.bounds.expand(bbPad), B = b.bounds.expand(bbPad);
	...
	function connected(a, b) {
		...
		const bc = b.bounds.center, ac = a.bounds.center;
		if (a.contains(bc) || b.contains(ac)) return true;
		const acp = a.clone(), bcp = b.clone();
		let u = null, ok = false;
		try {
			u = acp.unite(bcp);
			ok = Math.abs(u.area) < Math.abs(a.area) + Math.abs(b.area) - areaEps;
		} catch (_) { ok = false; }
		finally { if (u) u.remove(); acp.remove(); bcp.remove(); }
		return ok;
	}

	for (let i = 0; i < n; i++) {
		for (let j = i + 1; j < n; j++) {
			if (connected(items[i], items[j])) { adj[i].add(j); adj[j].add(i); }
		}
	}
	...
	for (let i = 0; i < n; i++) {
		...
		while (stack.length) {
			const k = stack.pop();
			comp.push(k);
			for (const m of adj[k]) { if (!visited[m]) { visited[m] = true; stack.push(m); } }
		}
		...
		if (offset > 0) {
			try {
				...
				const expanded = PaperOffset.offset(items[i], offset, {
					join, limit: miterLimit, insert: false
				});
	...
	for (const comp of clustersIdx) {
		...
		for (const idx of comp) {
			...
			if (!merged) { merged = p.clone(); }
			else { try { const u = merged.unite(p); merged.remove(); merged = u; } catch (_) {} }
		}
	...
	if (simplifyTolerance > 0) { for (const it of buffered) it.simplify(simplifyTolerance); }
	...
	if (mergeClusters) {
		if (buffered.length > 1) {
			...
			for (let i = 1; i < buffered.length; i++) {
				try { const u = acc.unite(buffered[i]); acc.remove(); acc = u; } catch (_) {}
			}
		...
		for (let i = 0; i < m; i++) {
			for (let j = i + 1; j < m; j++) {
				if (boundsIntersect(buffered[i], buffered[j]) &&
		...
		for (let i = 0; i < m; i++) {
			...
			const comp = [];
			const st = [i]; visited2[i] = true;
			while (st.length) {
				const k = st.pop(); comp.push(k);
				for (const t of adj2[k]) { if (!visited2[t]) { visited2[t] = true; st.push(t); } }
			}
			groups.push(comp);
		}
		const mergedGroups = [];
		for (const g of groups) {
			...
			for (const idx of g) {
				...
				if (!acc) { acc = it.clone(); }
				else { try { const u = acc.unite(it); acc.remove(); acc = u; } catch (_) {} }
			}
	...
	// Cleanup
	for (const u of unions) { try { u.remove(); } catch (_) {} }
	for (const eb of expandedBonds) { try { eb.remove(); } catch (_) {} }
	for (const ei of expandedItems) { try { ei.remove(); } catch (_) {} }
	cleanup();
	return results;
...
function extractOuterBoundary(item, scope) {
	...
	for (let i = 1; i < outers.length; i++) {
		try { const u = merged.unite(outers[i]); merged.remove(); merged = u; } catch (_) {}
	}
	item.remove();
