	...
	for (const child of svgElement.children) {
		...
		switch (tag) {
			...
			case 'circle': {
				...
				if (r > 0) shapes.push({ type: 'circle', cx, cy, r, hasFill, hasStroke, strokeWidth: sw, closed: true });
				break;
			}
			case 'ellipse': {
				const cx = parseFloat(child.getAttribute('cx') || '0');
				...
				if (rx > 0 && ry > 0) shapes.push({ type: 'ellipse', cx, cy, rx, ry, hasFill, hasStroke, strokeWidth: sw, closed: true });
				break;
			}
			case 'line': {
				...
				shapes.push({ type: 'line', x1, y1, x2, y2, hasFill, hasStroke, strokeWidth: sw, closed: false });
				break;
			}
			case 'rect': {
				...
				if (rw > 0 && rh > 0) shapes.push({ type: 'rect', x, y, width: rw, height: rh, hasFill, hasStroke, strokeWidth: sw, closed: true });
				break;
			}
			default:
...
export async function createSurroundingOutline({
	...
	const bondNodes = bondSelector
		? Array.from(svgRoot?.querySelectorAll(bondSelector) ?? [])
		: [];
	...
	try {
		...
		if (results.length === 0) {
			...
			await processIndividualShapes(
				allShapes, contourGroup, strokeColor, viewBox,
				defaultOutlineOptions, strokeWidth
			);
		} else {
 ...
 */
async function processIndividualShapes(allShapes, contourGroup, strokeColor, viewBox, outlineOptions, strokeWidth = 1) {
	const BATCH_SIZE = 8;
	for (let batchStart = 0; batchStart < allShapes.length; batchStart += BATCH_SIZE) {
		...
		const results = await Promise.allSettled(batch.map(async (elem) => {
			...
			const isBond = /** @type {SVGElement} */ (elem).classList
				? Array.prototype.some.call(/** @type {SVGElement} */ (elem).classList, (/** @type {string} */ cls) => cls.startsWith('bond-'))
				: false;
		...
		for (let idx = 0; idx < results.length; idx++) {
			...
			if (result.status === 'fulfilled') {
				...
				const elem = batch[idx];
				console.warn('[surrounding-outline] Error computing outline for single element:', result.reason);
				const directCopy = /** @type {SVGElement} */ (elem.cloneNode(true));
				applyDirectCopyStyle(directCopy, elem.tagName, strokeColor, strokeWidth);
				contourGroup.appendChild(directCopy);
			}
